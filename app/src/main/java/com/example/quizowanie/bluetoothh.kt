...
import kotlinx.android.synthetic.main.cos.*
import java.io.*
...
class bluetoothh : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.cos)
            ...
            val textV = findViewById<TextView>(R.id.textVieww)
        ...
        fun appendText(text: String) {
            runOnUiThread {
                this.textVieww.text = this.textVieww.text.toString() + "\n" + text
            }
        }
    ...
    class BluetoothServerController(activity: bluetoothh) : Thread() {
    ...
    class BluetoothServer(private val activity: bluetoothh, private val socket: BluetoothSocket) :
        Thread() {
        ...
        override fun run() {
            try {
                ...
                Log.i("server", "Czytanie")
                inputStream.read(bytes, 0, available)
                val text = String(bytes)
                Log.i("server", "Wiadomość otrzymana")
                Log.i("server", text)
                activity.appendText(text)
            } catch (e: Exception) {
                Log.e("client", "Nie można odczytać", e)
            } finally {
                inputStream.close()
                outputStream.close()
                socket.close()
    ...
    class SelectDeviceDialog : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setTitle("Wyslij wiadomość do ")
    ...
    class BluetoothClient(device: BluetoothDevice) : Thread() {
        ...
        override fun run() {
            Log.i("client", "Połączono")
            this.socket.connect()

            Log.i("client", "Wysyłanie")
            val outputStream = this.socket.outputStream
            val inputStream = this.socket.inputStream
            try {
                outputStream.write(message.toByteArray())
                outputStream.flush()
                Log.i("client", "Wysłano")
            } catch (e: Exception) {
                Log.e("client", "Nie można wysłać", e)
            } finally {
                outputStream.close()
                inputStream.close()
                this.socket.close()
